...
import SelectedMain from '../main/SelectedMain';
...
function SelectedNav() {
    ...
    const getIconSource = (buttonType) => {
        switch (buttonType) {
            case 'home':
                return location.pathname === '/selectedMain' ? homepink : home;
            case 'feed':
    ...
    return (
        <NavContainer>
            <OptionContainer>
                <NavLink to="/selectedMain" active={location.pathname === '/selectedMain'}>홈</NavLink>
                <Link ><NavButton src={getIconSource('home')} alt="홈" onClick={() => setActiveIndex(0)} /></Link>
            </OptionContainer>
            <OptionContainer>
